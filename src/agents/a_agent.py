...
class AAgent(Agent): # Inherit from Agent
    def __init__(self):
        super().__init__(
            id="agent_a_tool",
            name="A Agent 工具",
            ...
            tags=["影片", "剪輯", "摘要"],
            parameters=[
                {"name": "input_text", "type": "str", "description": "查詢內容，僅允許 10~100 字元中英文數字空白", "pattern": "^[\\u4e00-\\u9fa5a-zA-Z0-9 ]{10,100}$"} # Escaped unicode range
            ],
            example_queries=[
                "請幫我查一下影片剪輯教學",
                "什麼是 YouTuber？"
    ...
    def respond(self, input_text: str) -> dict: # ensure 'self' is present
        """回傳 A 網站摘要結果"""
        return {
            "type": "text",
