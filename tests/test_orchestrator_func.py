...
@patch("src.orchestrator.get_agent_list", return_value=[])
...
@patch("src.orchestrator.call_llm", return_value='{"tool_id": "test_tool", "parameters": {}}')
def test_dispatch_agent_single_turn_tool_func_exception(mock_llm, mock_tools):
    with patch("src.orchestrator.get_agent_list", return_value=[{"id": "test_tool", "name": "Test Tool", "description": "desc", "parameters": [], "function": lambda **kwargs: 1/0}]):
        result = dispatch_agent_single_turn("測試指令")
        assert result["type"] == "error"
